...
- **`_carga_activo_remata` → `SCORE_CARGA_ACTIVO_REMATE` (41900) / `_carga_activo_habilita_ataque` → `SCORE_CARGA_ACTIVO_ATAQUE` (31300)**: *atacar con el activo es lo primero*. La banda la fija `energy_score` (doc 10), pero esta rama la **protege** de sus propios overrides: el veto de primer turno y el cap por Bug Catching Set se exceptúan cuando la carga remata, y ni el `+200` de desempate (`plan.attacker == 0`) ni el downgrade a `7500` se aplican con cualquiera de las dos banderas puestas — el `+200` cruzaría la calibración de 31300 contra el motor UB (31450) y Teal Dance (31500), y el downgrade borraría el ataque del turno. Origen: user, episodio 88433181 registro_006 paso 67 vs Marnie's Grimmsnarl (GANADA con error).
...
- **`_attach_enable_retreat_attack` → `31200`** (log 88162794 turnos 11/13 vs Archaludon ex, PERDIDA 6-1 **sin atacar nunca** desde el turno 7): versión **no letal** de `_attach_enable_retreat_ko`. El activo (Meowth ex, 0 energía, coste 1) no puede atacar **ni retirarse** y el atacante de banca (Meganium a e4/e6) solo hace **chip** — Solar Beam 140 contra Archaludon ex de 300/400 PV, o 80 tras la resistencia Planta del Duraludon y Full Metal Lab —, así que la versión letal y **toda** la familia de pivotes de retirada (que exigen KO) quedaban muertas: la Planta se cargaba en el Meganium ya listo (Solar Beam es plano: +0 de daño) y se cerraba el turno, cuatro turnos seguidos. Si el activo no puede atacar de ninguna forma, el chip vale infinitamente más que 0: la energía va al ACTIVO a pagar la retirada. Condiciones (deck-agnósticas): retirada aún ilegal (`not can_switch`), `not can_attack`, la Planta completa el coste (`e < rc <= e + _grass_attach_unit()`), el activo **sigue** sin poder atacar con ella (`not _can_attack_eff`), el atacante de banca ya está listo **sin** esa energía (si es ella la que lo deja listo, su sitio es la banca) y, si el activo es un ex, el cuerpo que subirá aguanta al menos lo que le queda (`min_body_hp`, espejo de `_xx_vale` en `main-14`). Banda `31200`: sobre cualquier carga de banca (≤31150, incluida la de Ripening al mejor atacante) y bajo todo lo que habilita un KO este turno (31300+, 31500, 41000). Espejo en Teal Dance a **`31250`** (`o.area == ACTIVE`): si el activo que necesita la energía es el propio Ogerpon, su habilidad paga el coste **y roba**. Las dos ramas de Teal Dance aceptan además las banderas de la **ruta por habilidad** (`_ability_unlock_retreat_ko` / `_ability_unlock_retreat_attack`): *Teal Dance* adjunta aparte del adjunte manual, así que la línea sigue viva con `energyAttached` gastado — igual que en *Ripening Charge*.
...
1-bis. **`_carga_activo_remata` / `_carga_activo_habilita_ataque`** (doc 10): si el ACTIVO llega a su coste de ataque con la energía que aún puede moverse, y **este Ogerpon es el activo**, su Teal Dance hereda la banda (`41900` / `31300`) — adjunta la Planta **y roba**. Si el Ogerpon está en **BANCA** y la mano no da para ambos (`Plantas <= _carga_activo_falta`), se **veta**: Teal Dance solo se adjunta a sí misma, así que se comería la Planta que el activo necesita para atacar hoy (registro_006 paso 67 vs Marnie's Grimmsnarl).
2. `_td_ko_on_active` (o `_td_is_lethal_focus`) → **`31500`** — la energía habilita un KO inmediato.
2-bis. **`_grass_anywhere_enables_syrup_ko`** → **`31500`** — el Hydrapple ex **ACTIVO** ya ataca pero se queda corto y *Syrup Storm* cuenta la Planta de **todos** nuestros Pokémon: una Planta más **en cualquier sitio** lo vuelve letal (`_extra_energy_enables_ko(Hydrapple_ex, …)`). Este Ogerpon no necesita ganar nada con la energía, así que va **antes de los topes por matchup** — sin esto, un Ogerpon de banca ya cargado quedaba vetado por "sobrecarga" (paso 10) y la Planta moría en la mano (registro_006 paso 68 vs Mega Abomasnow ex: 330 → 390 sobre 350 PV).
...
1. Sin Planta en mano → `SCORE_VETO`.
1-bis. **`_carga_activo_remata` → `41900`** / **`_carga_activo_habilita_ataque` → `31300`** (doc 10): Ripening Charge es la vía que **completa el coste de ataque del ACTIVO** cuando el adjunte manual no basta o ya se gastó (adjunta aparte, así que sigue viva con `energyAttached` puesto). El objetivo — el ACTIVO — lo fija `energy_score`/`ATTACH_FROM` con la misma banda, así que la Planta aterriza donde debe (registro_006 paso 67 vs Marnie's Grimmsnarl: eran las dos habilidades las que llevaban la energía a un Ogerpon de banca mientras el Hydrapple activo se quedaba a 0 con el KO servido).
...
5. **`_ability_unlock_retreat_ko` → `41000`** / **`_ability_unlock_retreat_attack` → `31250`** (registro_014 paso 137 vs Alakazam): Ripening Charge que **desbloquea la retirada** del activo hacia un atacante de banca. Es el espejo exacto de las ramas homónimas de Teal Dance y de `_attach_enable_retreat_ko/_attach_enable_retreat_attack` del adjunte manual, pero por la **ruta de la habilidad**: como Ripening Charge adjunta aparte, la línea sigue viva con `energyAttached` ya gastado — que es justo cuando las banderas del adjunte manual se apagan y nadie más la veía. Ver `_grass_unlocks_active_retreat` (doc 02). **Activar la habilidad no basta**: el destino lo decide `energy_score`/`ATTACH_FROM` y hasta el registro_006 paso 101 no tenía rama para esta línea, así que la habilidad se activaba "por la línea correcta" y la Planta acababa en un cuerpo de banca (doc 10).
6. `_hydra_energy >= 2` (ya ataca): `29000` si `_extra_energy_enables_ko`; `30000` si el activo Hydrapple **no** noquea y hay banca cargable (`_active_hydra_cannot_ko and _bench_has_chargeable`); `30000` si `_tapu_future_charge` (dejar listo al Tapu de banca para el próximo turno); **`30000` si `_ripen_bench_ready_pivot`** — con el adjunte manual **ya gastado**, una Planta más deja LISTO a un atacante REAL de banca (`MAIN_ATTACKERS` + `_can_attack_eff`, nunca un Applin/Chikorita): todas las ramas anteriores solo miran si la Planta le sirve **al propio Hydrapple**, así que la habilidad se vetaba y la Planta acababa de forraje en el coste de una Ultra Ball. Acotado a `state.energyAttached` porque mientras el adjunte manual siga libre el ruteo normal ya coloca la Planta; resto → veto.
7. `_active_needs_energy` sin sobra, fuera del activo → `7500`.
8. Caso general: efectiva <2 → `31150` (banca a 0 energías) o `31100`; efectiva ≥2 → `30500`.
...
- **Orden de jugada por tiers (doc 15)**: `EVOLVE` siempre `_TIER_DEVELOP` (y por tanto sigue precediendo al Bug Catching Set, a diferencia de bajar un básico, que ahora le cede); `ATTACH` `_TIER_KO_ENERGY` solo si es la energía de KO exacta del plan (con la excepción `_tapu_future_charge` que la baja a `_TIER_ENERGY`); **Teal Dance** se promueve a `_TIER_ENERGY` (dentro del tier gana su score ~31500 sobre el adjunte ~31410); Ripening Charge y el resto de `ABILITY` quedan en tier 0.
...
- `_attach_enable_retreat_attack` a 31200 (vs Archaludon ex, log 88162794): el mismo adjunte habilitante cuando el atacante de banca **no remata**, solo hace chip. Cierra el agujero de "todos los pivotes exigen KO" que regaló cuatro turnos seguidos con un Meganium cargado mirando desde la banca.
