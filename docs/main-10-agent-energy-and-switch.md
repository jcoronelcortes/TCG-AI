...
Cuando la carta que activa su habilidad es Meowth ex, `_meowth_devel_lillie` está activo y ya hay una `Lillie_Determination` en mano, se prefiere **no** buscar otro Supporter (para no gastar el efecto en balde), salvo remate vía Boss's (`_win_via_boss_gust`/`_gust_2prize_via_boss`). La bandera se consume en las ramas `YES`/`NO` del bucle: normalmente `YES` en `ACTIVATE` puntúa 10, pero con `_meowth_skip_fetch` se invierte (`YES` → −1, `NO` → 10).
