...
- **`_carga_activo_remata` / `_carga_activo_habilita_ataque` / `_carga_activo_falta`** — *ATACAR CON EL ACTIVO ES LO PRIMERO*. Todas las banderas anteriores razonan sobre cargar un cuerpo de **banca** y promoverlo; faltaba la pregunta previa: **¿puede atacar el ACTIVO este turno si le llevo la energía que aún puedo mover?** Se calcula el **presupuesto real de carga hacia el activo** — adjunte manual (si no se gastó) más las habilidades que pueden apuntarle (`_grass_ability_slots_activo`: *Ripening Charge* de cualquier Hydrapple ex adjunta a quien sea; *Teal Dance* solo cuenta si el Ogerpon **es** el activo) — limitado por las Plantas de la mano. Si con ese presupuesto el activo alcanza su coste (`ATTACK_ENERGY_REQ`, con `_coste_de_ataque_min` derivado del dato de carta como respaldo) y el ataque hace daño, la carga va al ACTIVO: **`SCORE_CARGA_ACTIVO_REMATE` (41900)** si noquea, **`SCORE_CARGA_ACTIVO_ATAQUE` (31300)** si solo hace chip y sin esa carga el turno sería **estéril**. Guardas: nada de dormido/paralizado/confuso (`condition_blocks_action`), el KO debe ser garantizado (`_ko_no_garantizado`), y el remate **cede ante un KO más barato** — si `_attach_enable_retreat_ko` / `_ability_unlock_retreat_ko` ya detectaron un atacante de banca cargado al que solo le falta que paguemos la retirada, esa línea cobra el mismo premio con UNA Planta y además pone a salvo al activo. La variante sin KO no dispara con banca lista y promovible (`_bench_attacker_ready and can_switch`) ni vs Cubchoo (allí el rival bloquea nuestro ataque y la energía se reserva para retiradas). Origen: user, episodio 88433181 registro_006 paso 67 vs Marnie's Grimmsnarl (GANADA con error) — Hydrapple ex activo recién evolucionado a 0 energías, tres Plantas en mano, adjunte manual sin gastar y dos *Ripening Charge* vivas contra un Munkidori a 10 PV; el agente cargó al Hydrapple de **banca** y mandó las habilidades a un Ogerpon de banca: turno sin atacar con el KO servido.

- **`_ogerpon_lethal_focus_serial`** — concentra la carga en **un** Ogerpon que este turno puede volverse letal (41700), vetando cargar a otro. **No se activa cuando la Planta tiene un destino más urgente: pagar la RETIRADA del activo** (`_ability_unlock_retreat_ko` / `_ability_unlock_retreat_attack`). Motivo (user, registro_006 paso 101 vs Alakazam, PERDIDA): con un Ogerpon ex de banca a 6 energías efectivas — ya letal sobre el Alakazam — atrapado detrás de un **Applin activo a 0 energías**, el foco mandaba la Planta al **otro** Ogerpon "para volverlo letal", fabricando un segundo rematador igual de atrapado; el turno murió sin atacar. Mientras la retirada no esté pagada, cargar banca no promueve a nadie.
...
### Destino de la línea "Planta al ACTIVO → RETIRAR → atacar con el de banca" (41000 / 31250)

Inmediatamente después de `_carga_activo_remata` (y **antes** del foco de Ogerpon), si el objetivo es el **activo** y alguna de las cuatro banderas de la línea está encendida — `_attach_enable_retreat_ko` / `_ability_unlock_retreat_ko` → **41000**, `_attach_enable_retreat_attack` / `_ability_unlock_retreat_attack` → **31250** — la carga va al activo.

> **El acto de cargar y el DESTINO de la carga son dos decisiones distintas** (user, registro_006 paso 101, episodio 88492701 vs Alakazam, PERDIDA). Las cuatro banderas ya existían y puntuaban el *acto*: el adjunte manual en su rama de `OptionType.ATTACH`, *Teal Dance* y *Ripening Charge* en las suyas (doc 13). Pero el **destino** se decide aquí, en `energy_score` — y aquí no había nada: el activo caía en la banda genérica de desarrollo (~8000) y cualquier cuerpo de banca le ganaba. Resultado en el registro: el agente activó *Ripening Charge* **por la línea correcta** y acto seguido mandó la Planta a un Ogerpon de banca, dejando al rematador real (otro Ogerpon ex, a 6 energías efectivas) atrapado detrás de un Applin que no podía retirarse; turno estéril y partida perdida. Las bandas son las mismas que ya citaban los consumidores. Regresión: `tests/test_retirada_hacia_el_rematador_de_banca.py`.
