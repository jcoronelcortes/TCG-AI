...
Devuelve `(ko, chip)`: **¿las Plantas que aún pueden aterrizar sobre el ACTIVO pagan su coste de retirada y habilitan atacar con un cuerpo de banca?** Es el núcleo común de la línea *"Planta al activo → RETIRAR → atacar con el de banca"*, y lo consumen **dos rutas distintas**:
...
Devuelve `(False, False)` si no hay nada que desbloquear: el activo ya paga su retirada (`e >= rc`), las Plantas del **presupuesto** no le alcanzan (`need = ceil((rc - e) / _grass_attach_unit()) > budget`), o con esas Plantas **el propio activo ataca igual o mejor** que el cuerpo de banca. El `chip` solo se evalúa si el activo no puede atacar este turno y aplica la guarda "no cambiar un ex por un cuerpo peor" (`min_body_hp`). El Grass del campo tras retirar se aproxima descontando `_retreat_grass_units(rc)` (la retirada descarta cartas enteras).

> **El presupuesto de carga, no "una Planta"** (user, episodio 88631738 paso 77). El detector medía exactamente **una** unidad (`e + unit < rc → (False, False)`), así que cualquier activo atrapado con coste de retirada de **2 o 3 símbolos** era invisible aunque la retirada fuese perfectamente pagable con las vías vivas del turno. El parámetro `budget` es el mismo cálculo que ya usaba `_carga_activo_remata` para el coste de **ataque** del activo — adjunte manual si sigue libre + `_grass_ability_slots_activo` (Ripening Charge de cada Hydrapple ex, Teal Dance solo si el Ogerpon **es** el activo) —, acotado por las Plantas de la mano. La cadena se ejecuta paso a paso y el desempate greedy vuelve a evaluar tras cada carga, con `need` ya decrementado, así que las dos vías se encadenan solas. El **suelo de 1** conserva el comportamiento histórico para los consumidores que traen su Planta de fuera de la mano: la Night Stretcher la recupera del **descarte**, donde `hand_counts` es 0 y la línea de una sola Planta debe seguir viva. Regresión: `tests/test_retirada_hacia_el_rematador_de_banca.py`.

> **La comparación con el ataque del activo es por DAÑO, no por "puede atacar"** (user, registro_006 paso 101, episodio 88492701 vs Alakazam, PERDIDA). El guardia original era `if _can_attack_eff(act.id, e + unit): return False, False`: bastaba con que el activo *llegase a su coste de ataque* para apagar la línea entera. En el registro el activo era un **Applin** (coste de ataque 1, coste de retirada 1) y en la banca había un **Teal Mask Ogerpon ex a 6 energías efectivas** que noqueaba al Alakazam (Myriad Leaf Shower 240 sobre 140 PV) — y a `_attacker_base_damage` el Applin ni siquiera le figura, o sea que el "ataque" que vetó el KO valía **0 de daño** para el propio modelo. Ahora se calcula el daño **efectivo** del activo con esa misma Planta y solo se aborta si **remata** (atacar con el activo es lo primero) o, en la vía de `chip`, si iguala o supera lo que haría el cuerpo de banca (`chip > act_dmg`: el empate se queda con el activo, que además no gasta la retirada). Regresión: `tests/test_retirada_hacia_el_rematador_de_banca.py`.
