...
### PESCA DE REMATE — `pescar_energia_para_remate` (5900)

La regla que **anula los vetos de orden** de Lillie's (`ultra_ball_completa_linea`, `cede_a_boss_ejecutable`, `linea_pendiente`) cuando el robo es la **única línea que ataca este turno**. User, registro_004 paso 49 vs Marnie's Grimmsnarl (PERDIDA): Teal Mask Ogerpon ex activo con 1 de las 3 energías de *Myriad*, banca sin cargar, **cero** Plantas en mano y 6 premios intactos — Lillie's roba OCHO, y con 2 Plantas *Myriad Leaf Shower* pega 30 + 30 × (3 propias + 2 del rival) = 180, ×2 por debilidad Planta = **360 ≥ 320**: dos premios. El agente jugó Boss's para arrastrar un Snorunt de 70 PV, y con el Supporter gastado la Ultra Ball descartó las dos Lillie's.

Quién decide: `_pesca_de_remate` (ver `main-02`) calcula el plan y su probabilidad hipergeométrica sobre la creencia de mazo; `_pesca_remate_valida` es el *gate* compartido con Boss's. Exige **todo** esto: hueco de Supporter libre; **ningún** ataque posible hoy (`not can_attack and not has_ready_bench_attacker` — si algo puede atacar, manda el ladder normal); el plan es **letal** y cobra ≥ `PESCA_PREMIOS_MIN` premios con `prob >= PESCA_PROB_MIN` (0.35); ninguna evolución directa pendiente (`pending_evo`: se juega antes por tier, así que ceder no cuesta el remate, y si hoy no es jugable barajar sus piezas sí costaría la línea); y ningún remate **seguro** en juego (`win_via_boss_gust`, `boss_win_via_bench`, `boss_dodge_redirect`, `win_ko_active_via_promote`), que siempre gana a un KO probable. Los vetos **duros** de más arriba en `_REGLAS_LILLIE_PLAY` (Supporter jugado, Sello pendiente, freno de deck-out, guards de Xerosic, vs Comfey) siguen mandando: la pesca se coloca después de ellos.

`LILLIE_SCORE_PESCA_REMATE` = 5900, por encima del Lillie's hydra-cargado (5800) y de todo el ladder de Boss's que no gana la partida ni cobra 2 premios ya. Boss's, además, cede explícitamente (`cede_a_pesca_de_remate`, `main-08`): gustear cambiaría el activo rival justo cuando ese activo **es** el objetivo del remate que se pesca.

Medición (self-play, jul 2026): espejo con la regla vs sin ella **50.1% [48.9–51.4] con n=6000** — neutra en agregado; barrido por matchup a n=1000/rama con deltas dentro del ruido (marnie −0.2, alakazam +0.7, crustle +0.2, dragapult +0.8, hops −0.6, comfey −0.7, cynthia −0.4, festival +0.2). Frecuencia de disparo: **5–19% de las partidas** según matchup. El corpus dorado no registra flips. Es decir: **arregla la línea concreta sin daño colateral medible**; el bot del harness no premia una decisión de un turno, así que el gate solo sirve aquí para descartar regresión.
