...
### The parallel session, and why the hard rule earned its keep

This night ran alongside another session working on the same tree. That is not a
complaint, it is a measurement: three commits landed on `main` while the plan
was being written (which is how Q1 answered itself), and near the end the full
suite went **red with five failures in `tests/test_selfplay.py`** — while the
same file passed in isolation seconds later, and while `utils/selfplay.py` sat
modified and uncommitted in the working tree.

The red was a half-written file being read mid-edit. Verified two ways: the
failing file passes alone, and `git archive HEAD` into a clean directory runs the
whole suite green except for three tests that need a git repository the export
does not have.

**This is precisely the failure mode the export of §2 exists to prevent**, and it
happened to the suite rather than to a measurement only because every M block was
already reading from `log/noche-2026-08-12/tree/`.
...
## Addendum, same night, after §8 was written — B7 closed by frequency

§8 listed B7 (the 280 Ripening ↔ Teal Dance ties) as open with a note: *"C2's top
row may be the same axis: read them together."* They were read together, and the
axis closed.

**They are the same axis.** Resolving the labels inside the 11 August permutation
dumps (`chose` / `chose_permuted`) gives the composition nothing had printed:

     415  ABILITY:Hydrapple ex     vs  ATTACH:Grass -> Hydrapple ex  (measured, reverted)
     280  ABILITY:Hydrapple ex     vs  ABILITY:Teal Mask Ogerpon ex  <- B7
      99  ABILITY:Hydrapple ex     vs  ATTACH:Grass -> Meganium
      87  ABILITY:Fezandipiti ex   vs  ABILITY:Teal Mask Ogerpon ex

**367 ties are "an ability outranks the Teal Dance"**, and the fourteen boards
C2 v2 counts as real losses are that same body being outranked. Two instruments
that do not talk to each other — a permutation probe (score ties, menu order
decides) and a tier-inversion census (tier decides before score) — land on one
question.

**The card text is what makes it a decision.** Both abilities attach a Basic {G}
Energy *from your hand*: Ripening Charge to any of our Pokémon plus a 30 heal,
Teal Dance to itself plus **a card**. With two Grass both happen and the order is
free. With one, the order is the whole decision — and on the 11 August dumps,
142 of the 280 had exactly one Grass, the Ogerpon was the ACTIVE in **142 of
142**, at two energies (one short of Myriad) in 138, and **intact in 136**. On
those, Ripening's heal is worth zero on its natural target and the trade is
"heal an undamaged body" against "draw a card".

**And then the current tree said no.** Replayed over the frozen corpus:

    both abilities offered        231 menus
    with exactly ONE Grass        136
      Teal Dance wins              63
      Ripening wins                49   <- and the Teal Dance scores -1 in ALL of them
      exact tie                    24
        ... tied at -1             16   (both vetoed: not a decision)
        ... a real tie              8

**8 of 3 580 decisions, 0.22 %** — twice below this project's own 0.5 % floor.
Where Ripening wins, the Teal Dance is *vetoed*, not competing: "the menu order
decides it" no longer describes this tree. B7 closes the way the cap and the
cost/search axis closed, by frequency, with the number written down.

Two honesty notes. The 280 came from 4 000 self-play games and the 8 from the
frozen corpus — different workloads, and what would settle it outright is
`utils/permutation_probe.py` re-run on today's tree, deferred because another
session was mutating it. And this family has already eaten one plausible
hypothesis: the sibling 415 ties were measured and reverted because their premise
had two errors.

---

## Addendum 2 — Festival Lead's 55 drifts are a documented exclusion, not a hole

§7 called Festival Lead "the corner where the damage model is least modelled"
and put it on the pending list. **That framing was wrong, and the correction is
the more useful result.**

Re-run with a dump, 200 games, HEAD: **70 findings, 70 of 70 on the seat piloting
their deck**, and every damage drift is exactly **30 or 40**:

    DAMAGE_DRIFT: 61   predicted 130 -> resolved 90   (40)
                       predicted 250 -> resolved 220  (30)
    MISSED_KO:     9   predicted 10-30 hp left; it was knocked out

Those two numbers are cards:

| card | effect | lists |
|---|---|---|
| Black Belt's Training (1211) | +40 vs our Active **{ex}**, this turn | 9 |
| Kieran (1191) | +30 vs our Active **{ex}**, this turn (one of two modes) | 5 |

Both are **already excluded in writing** by `utils/op_buff_census.py`, with the
same argument: *they last "during this turn", they live in the opponent's HAND
until played, and — for Kieran — the opponent chooses which of the two halves to
use.* That census reports `SIN MODELAR = 0` and has done for months.

So the oracle and the buff census agree: the residue is the price of a decision
already taken, not an unmodelled corner. Modelling it means predicting a card we
cannot see, in a hand we cannot read. **Nothing to fix, and the pending item is
withdrawn.**

### What it did expose was a defect in C1

The card-text census reported **Kieran as `NUNCA REFERENCIADA`**, because no
module names it. True, and useless: it takes a decided question and puts it back
on a worklist of open ones, where a reader spends the same attention on it as on
Deluxe Bomb and finds an argument already written.

C1 now reads the `_EXCLUDED` dict of each sibling census and gives them a band:

    NUNCA REFERENCIADA=86, SOLO NOMBRADA=3, EXAMINADA Y EXCLUIDA=5, MODELADA=133

Five cards moved out of the accusation band with their reason attached: Premium
Power Pro (15 lists), Black Belt's Training (9), Kieran (5), Acerola's Mischief
(4), Secret Box (3). An exclusion is not a promotion — they still rank above
MODELADA, because "we decided not to model it" is worth revisiting when the meta
rotates. It just is not an oversight.

This is the same lesson the night of 11 August already paid for, arriving from a
new direction: **a detector must compare against the finest reading the project
has, not the coarsest.** Two of the four detectors built this night have now
been corrected by that rule.

---

## Addendum 3 — B2, "the largest capability hole", is 1 in 8 525 menus

§6 left B2 not started and called it the largest capability hole on the list. It
had been open since 10 August, named in three separate write-ups, and **never
measured**. Its own plan entry said census first. Here is the census.

**What the class actually is.** The projector is for attackers whose damage
counts BODIES. On our side that is exactly one attack: *Do the Wave* (Dipplin),
20 × our bench. Myriad Leaf Shower counts the ENERGY on both actives and Syrup
Storm counts Grass — benching a body moves neither. So the population is every
menu where a Dipplin is active with a free bench seat and a Pokémon in hand.

**Frozen corpus, 3 580 decisions:**

    menus MAIN                        2097
    Dipplin active                     146
    ... with a Pokemon playable          32
    ... where ONE body crosses the KO     1

**Self-play, 240 games against four archetypes, 8 525 menus:**

    Dipplin active                     551
    ... with a Pokemon playable        158
    ... where ONE body crosses the KO   28
    ... AND it benches the expensive body with a cheap one available   **1**

**One in 8 525 menus, 0.012 %** — forty times below the 0.5 % floor this project
set itself. And the single case is arguable on its own terms: the hand held two
Teal Mask Ogerpon ex (2 prizes) and a Tapu Bulu (1 prize), and preferring the
Ogerpon over a body that needs four energies to attack is a defensible play for
reasons that have nothing to do with the threshold.

**The corpus board is not a defect either.** `registro_035_dragapult_3` turn 4:
our Dipplin at bench 4/5, their Drakloak at 90, Do the Wave 80 → 100 with one
more body. The menu ranked EVOLVE Hydrapple ex (33 000) over PLAY Applin
(21 900), and both live in `_TIER_DEVELOP`. Following the turn to the end:

    t4 a7   EVOLVE
    t4 a8   PLAY Applin        <- the body that crosses the threshold
    t4 a16  ATTACK

The Applin went down one action later and the attack happened. **The tier ordered
the turn; it did not exclude anything** — the same lesson C2 v2 taught two hours
earlier, arriving from a third direction.

**B2 closes by frequency.** What the original case (`registro_005` step 50)
showed was real, and the reading of it was correct; what was never checked is how
often the board recurs. It does not. Building a hypothetical-board projector for
one decision in 8 525 is precisely what this project's own rule forbids: *do not
ask a winrate of a 0.1 % event*, and this is an order of magnitude below that.

Scope stated plainly: 240 games against alakazam, dragapult, marnie and
festival_lead, plus the 50-record corpus. A wider sweep could raise it, and the
harness is a twenty-line sink over `TIER_CENSUS_SINK` if anyone wants to.
