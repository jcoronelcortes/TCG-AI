...
**Two blind spots fixed on 16 August 2026, and both had returned "there is no
better line" about a turn the agent itself plays** ([the write-up](alakazam-the-seat-that-closes-the-game-is-not-a-tie-break-2026-08-16.md)):

- **Wild Growth in the simulated charge.** The observation lists *effective*
  ...
  line came up one symbol short per charge, precisely on the boards where the
  doubler **is** the plan: a Meganium at 2/4 still read 3/4 after attaching, and
  its attack never appeared.
- **Lana's Aid.** The only recovery modelled was Night Stretcher, so on a board
  with ten Grass in the discard and none in hand the model said "no line" about a
  turn that wins. It is a Supporter, so it shares the slot with Boss's Orders,
  and that now branches.

The lesson generalises past this tool: an instrument that answers *"was there a
better line?"* has to model the deck's own engines, or its negatives are about
itself. Its test carries two controls (no Meganium on the board → a charge is
worth one; from the seat the episode chose → there really is no winning line),
and it does **not** hang off `records/`, which is transient.
...
| `gate_the_relay_inherits_the_seat.py` · `census_the_relay_inherits_the_seat.py` | The gust that sells a locked seat instead of jamming it. Census **0.05 flips/game** vs `marnie_grimmsnarl_1` with **10 of 10** taking a strictly bigger prize and no knock-ons; 0.00 vs `alakazam_1`, where a FORBID sits above the rung. The census is the pair that shows what closing the PLAY half is worth: 0.03 with only the target selector changed, 0.05 with both. |
| `gate_the_setup_seat_did_not_spend_the_ability.py` · `oracle_the_setup_seat_did_not_spend_the_ability.py` | The seat the SETUP gave, which never spent a come-into-play ability. Its `--census` is the `shadow.py` harness rather than the corpus (the corpus is 0 of 3 580): **0.54 decisions per game read differently, 0.01 changed** — our starting active is the ability's own body in one game in twelve. The oracle is where it is graded, and it is the entry that shows **the opponent's policy flipping the sign**: +1.89 of prize margin with their seat playing our agent, −0.50 with it playing random, winrate saturated (≈99 %) in both. |
| `gate_the_seat_the_search_buys.py` | The Poké Pad's two halves reading a Basic still in the DECK as evolvable today under Forest of Vitality. Corpus **0 of 3 580** and 0 golden flips — the rung is purely additive once its tie-break is stated — so what carries it is the live exposure instead: **0.027–0.040 fetches/game** across dragapult / alakazam / crustle_kangaskhan. Its `--census` prints the "the event is RARE" warning and means it. |
| `gate_the_gust_that_ends_the_game.py` | The Boss's Orders target that closes the game, in the ladder that only knows how to jam. Corpus **0 of 3 580** frozen and **1** golden flip (the reported decision), so the number that carries it is the live one — and it is reported **with its denominator**: **5 firings over 461 candidates the jam ladder priced**, in 1 200 games across four lists. That denominator is the point. A bare zero cannot tell *never at match point* from *the ladder never ran*, and a census that cannot say which it means is reporting its own blind spot. `--live-census` is the flag; `--census` walks the frozen corpus and prints the RARE warning. |
| `census_the_front_spot_goes_to_the_attacker.py` · `gate_the_front_spot_goes_to_the_attacker.py` | The forced promotion that went to a body which neither attacks nor steps aside. Harvested corpus 254 decisions → **1 flip** (the reported step), frozen **0 of 3 580**, census **4 forced promotions across 14 records**. The pair is worth opening for the *boundary* it enforces rather than the number: two wider versions of the same sentence each lose a board the project has already paid for. |
| `gate_the_front_seat_vs_alakazam.py` · `census_the_pivot_promotes_the_body_it_pays_for.py` | The two halves of the Alakazam pivot — whether the front seat is sold at all, and then to whom. Census **7 decisions of 2 416** across 32 Alakazam games with **0 of 3 940** outside the matchup; frozen **1 flip of 3 580**, reviewed and named. Note the flag on the leakage half: `--outside`, **not** `--all-matchups`, which includes the matchup itself and therefore cannot show leakage. |
| `census_marnie_the_engine_before_the_line.py` · `gate_marnie_the_engine_before_the_line.py` | The gust that cuts the Marnie **engine** instead of her line. Census **0.02 flips/game** with **6 of 7** the exact sentence; leakage **0 of 39 781** vs `crustle_wall_1`; gate n=1500 **−0.13 pp (z −0.18)** over a **+0.00 pp exact** control. The control at exactly zero is what makes the row readable: with no harness noise, −0.13 pp *is* two games of 1 500, which is what a handful of flips in either direction looks like. It is the entry that says out loud **the matchup the gate simulates is not the matchup the rule was written for** — the bot does not play *Adrena-Brain* like a person. |
| `census_the_gust_reads_the_energy_not_the_hp.py` | The order **inside** that ladder, and the jam chain that was missing it. Own census **0.007 flips/game — below its own written criterion, and the page says why**: the bot never builds the tied-HP board the real opponent did. The parent census is the other half of the reading: from 0.02 to 0.04 flips/game with the jam chain included, which is what establishes that chain was not a rare corner. The two are **not** the same seeds with one rung moved, so the doubling reads as an order of magnitude and never as a delta. |
| `census_the_bare_line_asks_nothing_of_the_reserve.py` · `gate_the_bare_line_asks_nothing_of_the_reserve.py` | The third half of the same engine: a line at zero energy is owed no reserve question. Census **0.03 flips/game — above** its pre-written 0.01 criterion — over a shape row of **0.23 bare-line gust menus per game**, zero knock-ons, and zero leakage on two non-Marnie lists. Gate n=4000: **+0.00 pp exactly against a control row of +0.00 pp exactly**, prizes +0.002. Read it as the model of a **neutral result reported as neutral**: a hundred boards changed and not one game's winner, which is the expected outcome against a bot that does not punish what the rule defends. |
| `census_the_seat_that_closes_the_game.py` | The promotion at **our** match point. Local 1 of 7 forced promotions; self-play 500 games vs `alakazam_1` → 584 promotions, **18** at match point, 7 already had a knocker, **0** with a named finisher. Golden 0, frozen 0 of 3 580, flag on/off diff 0, seeded paired gate 600 games **0 divergent**. Both arms are the same agent against the bot, so the file states plainly that **no winrate can resolve it** and what is adopted is the guarantee. |
| `census_the_seat_that_loses_the_game.py` | **The census that found half of its own rule.** Measuring the retreat half, its `doomed` counter showed 2 of 19 simulated promotions vs `crustle_wall_1` giving the seat away — a second menu nobody had read. Retreat: frozen 3 of 931 menus / 0 flips, self-play 0.59–0.97 % vs crustle and starmie, **zero** vs alakazam and dragapult, every flip ATTACK/PLAY/PASS → RETREAT with 0 escapes. Promotion: frozen 6 of 180 menus / 0 flips, self-play 0.22–4.71 %, and `P.saved == P.flips` in every run. The gate changes sign between runs and is therefore reported as reading nothing. |
| `census_the_ko_that_ends_the_game.py` | **Each arm plays its own games**, because the fix deletes the population it would be counted over — measuring on the fixed agent returns zero and proves nothing. 120 games per arm across four lists: `diverted` **6 (0.3 %) → 0**, `thrown` **5 → 0**. The baseline row is the instrument demonstrating it **can** fail before it reports the zero, which is the whole reason the zero is believable. |
| `census_the_top_in_play_does_not_close_the_line.py` · `gate_the_top_in_play_does_not_close_the_line.py` | A candidate **measured and not adopted**, shipped behind a `False` switch. The census wraps `_eval_ub_best_target` and **neutralises the switch on itself**, so it always measures the exposure and never the post-fix world; its unit is **distinct turns, not valuations**, because the record's board is priced eleven times inside one turn. 51 valuations → 32 dead → **29 with a full bench**; self-play 0.09–0.16 turns/game. Gate n=1000 × 4 lists: **−0.70 pp (p 0.057)** against a `--control` of **+0.25 pp**. See [the write-up](ultra-ball-the-top-in-play-does-not-close-the-line-2026-08-16.md) for the band it points at instead. |
...
| Tool | Purpose |
| --- | --- |
| `lint_architecture.py` | **Twelve** architecture rules, checked by the test suite. They cover failures that do **not** show up as a red test. R1–R5 watch the agent: importing a mutable by name (freezes a stale copy), data modules touching state, anything bound after the agent entry point (breaks the competition loader), lazy imports that break the container, one name defined twice. R6–R8 watch the INSTRUMENTS, and each comes from a bug that shipped on 10 August 2026: a test may not read a `records/` file without a skip guard (that directory is re-harvested), a two-arm `gate_*.py` must define **and call** `provenance()` (a gate that cannot see its own change reports neutral, and neutral orders a revert here), and inside the DISCARD block the turn-scoped flags may only be read through the horizon (on a forced discard they are the opponent's). R9–R10 watch the agent's own discipline (a per-option scorer may not write state; a computed field nobody reads is a question asked and ignored), and R11–R12 keep the instruments away from the submission and the **rollout side stateless** — nothing under `ptcg/search/` or `ptcg/opponent/` may import `AGENT_STATE` or `main`, because a stateful policy corrupts the live game's belief and pilots both seats of the simulation with it. The full table is in [Testing §7](testing.md). |
