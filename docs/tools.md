...
### `matriz_matchups.py` — the matchup matrix
...
`deck/rivales_reales/`. `--pesos` weights each list by how often it actually
...
### `explorador_turno.py` — exhaustive turn explorer
...
### `sonda_muro.py` — the immune-wall probe
...
| `grabar_corpus.py` | Records fresh games against the real leaderboard decks, in the same format, so the golden corpus can be regenerated without depending on downloaded replays. |
