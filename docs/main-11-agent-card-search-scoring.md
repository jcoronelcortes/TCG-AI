...
- Vs Crustle, gustear un `Dwebble_*` → `SCORE_FORBID`, **salvo `c.muro_bloquea_activo and c.can_ko`** (episodio 88620891 paso 78, PERDIDA). `muro_bloquea_activo` es deck-agnóstico: mide con `_our_effective_damage` que nuestro ACTIVO hace **0** al activo rival (Mysterious Rock Inn, Cornerstone Stance, Sylveon…), no una lista de ids. Con el muro delante, atacar de frente no cobra premios y un Dwebble noqueable es el único premio del turno (y además niega un Crustle futuro); el veto original (log 86339758) sigue vigente en cuanto exista algo mejor que hacer o el KO no sea real.
...
Su hermano de **un turno más allá** es `_sin_atacante_para_manana` (doc 04): sobre los cuerpos que YA están en juego cuenta el adjunte del PRÓXIMO turno (una Planta más, en unidades efectivas) y las evoluciones que la mano puede completar sobre una pre-evo en mesa (la evolución hereda la energía del cuerpo). No cuenta los Básicos de la mano —un Tapu Bulu recién bajado necesita 4 energías, no una— y solo mira `MAIN_ATTACKERS`: un Chikorita que pega 10 no es "empezar a atacar". Es conservador por diseño (ante la duda devuelve `False`, "sí tenemos atacante"), porque quien lo consulta lo usa para justificar gastar recursos.
...
0. **PRIMER TURNO = SOLO LILLIE'S** (`primer_turno_solo_lillie` / `primer_turno_resto_cede_a_lillie`, las dos primeras reglas de la cadena; user, log 88461779 paso 16 vs Alakazam, PERDIDA): en NUESTRO primer turno `Lillie_Determination = 1400` y **cualquier otro Supporter se capa a ≤40**. El turno 1 no ataca, no evoluciona y —saliendo primeros— ni siquiera ofrece jugar Supporters: lo único que decide es cuánta MANO tendremos el turno 2, y todo lo demás se queda muerto en la mano (o se baraja con la propia Lillie's del turno siguiente). En aquel log el fetch se llevó un Xerosic (rama 4) con cuatro Lillie's vivas en el mazo. El degradado del resto exige `lillie_alcanzable` —la Lillie's está de verdad entre las opciones del prompt (o viva en el mazo, cuando `_meowth_fetch_prediccion` predice el fetch antes de bajar el Meowth)—, así que en un mazo sin Lillie's la regla no capa a nadie y decide la escalera normal.
