...
- `utils/grabar_corpus.py` — genera los registros que alimenta el corpus dorado, jugando contra los mazos REALES del leaderboard (`deck/rivales_reales/`) con el bot genérico y alternando asiento. Quita la dependencia de tener replays de Kaggle en `log/`: sin él, el corpus queda ciego y su test sale como *skip*. `python utils/grabar_corpus.py --partidas 12` y después `python tests/golden_corpus.py --actualizar`.
