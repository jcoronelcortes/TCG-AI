...
| `grupos.py` | Derived groupings: which cards are walls, threats, pre-evolutions of ex lines, and so on. |
| `lineas.py` | Evolution lines: stage of a card, its root basic, the chains our deck can build, and which link is missing. |
| `costes.py` | Printed attack costs of our attackers. The *effective* cost of the turn is derived from this (a stadium can tax it). |
| `tablas.py` | Card and attack tables built once from the simulator's data. |
| `puntuacion.py` | Scoring constants shared by several phases. |

### `ptcg/motor/` — the decision scaffolding

| Module | Purpose |
| --- | --- |
| `plan.py` | `AttackPlan`: the turn's chosen attacker, target and attack. |
| `contexto.py` | `DecisionContext`: the snapshot of the turn handed to the per-card scorers. |
| `reglas.py` | A small generic rules engine (fixed rules, adjustments, scenario resolution). |
| `depuracion.py` | Decision dumping for debugging. See [Debugging a decision](debugging.md). |

### `ptcg/calculo/` — reading the board
...
| Module | Purpose |
| --- | --- |
| `energia.py` | Effective energy: our Grass energy counts double while our accelerator is in play, and a stadium can raise attack costs. Answers "can this Pokémon attack?". |
| `dano.py` | What our attackers actually do: base damage, whether a knockout is guaranteed, sniping the bench. |
| `carta.py` | Reading a card from the observation: its prize value, how good a body it is. |
| `rival.py` | Reading the opponent: can it attack, which of its bodies are harmless, how big is its hand. |
| `tablero.py` | Board reading: our active, what can evolve, what the hand can do. |
| `planta.py` | The energy plan: how many more Grass energies the board can actually use, and whether one of them unlocks an attacker today. |
| `probabilidad.py` | Draw probability, used to price refills and searches. |

### `ptcg/estado/` — memory

| Module | Purpose |
| --- | --- |
| `agente.py` | `ESTADO`: the mutable state that survives between turns (the plan, who goes first, whether we got knocked out, pending search commitments, opponent-archetype flags). One object, never reassigned. |
| `tracking.py` | The belief about our own deck: initial scan, card movement between zones, prize identification. |
| `claves.py` | The zone keys used by that tracking (deck / hand / in play / discard / prize). |
...
| `disrupcion.py` | Hand disruption. Xerosic's Machinations and Unfair Stamp live together on purpose: the correct order between them makes each consult the other. |
...
| `estadios.py` | Stadium play: our own accelerator, and the shared instant-evolution stadium. |

### `ptcg/turno/` — the phases of a turn

This is the body of what used to be one enormous function, split by phase.

| Module | Purpose |
| --- | --- |
| `puntuacion.py` | The dispatcher: sends each menu option to the branch for its type. |
...
| `energia.py` | Who deserves the energy attachment this turn. |
