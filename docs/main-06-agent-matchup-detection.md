...
- **Límite conocido de `active_ko_likely`**: su insumo `_op_best_damage_vs` recorre `card.attacks`, que son **ids** (ints), y hace `getattr(id, 'damage')` → siempre `None`, así que el daño impreso rival **nunca se lee** y la condición (1) casi no dispara: un activo a vida completa frente a un Mega Lucario ex con las 2 energías de *Mega Brave* (270) quedaba marcado como "a salvo" (registro_004 t4, PERDIDA). Por eso las reglas que exigen certeza usan `_active_doomed_real` (`_op_active_attack_damage_to(op_active, my_active, op_state.handCount) >= hp`, que **sí** resuelve el ataque por `attack_table`) y se expone en el `DecisionContext` como `active_doomed_real` para las cesiones de Boss's/Lillie's (`main-04`).
