"""THEIR match point at the promotion, graded by the ENGINE'S RULES.

WHY THIS EXISTS. `PROMOTE_BET_OUTLIVES_MATCH_POINT` lifts the two match-point
vetoes off ONE body: the finisher `_promote_setup_ko_attacker` already named,
when it can pay its own retreat and there is a cheaper body to walk back into.
It fires on a board the reference bot rarely builds -- our active knocked out,
their pile at or below what our almost-ready finisher costs, and that finisher
carrying the energy for its own exit -- so a winrate cannot resolve it: the
frozen fifty produce ZERO flips and the harvested records exactly one
(`registro_006` step 77 vs Archaludon ex, episode 92848103).

So the question is asked the other way round, the way its sibling
`utils/oracle_the_promotion_bets_when_it_can_walk_back.py` asks it:
...
`PROMOTE_BET_OUTLIVES_MATCH_POINT` rebound to False in one arm, both corpora
...
Usage:
    ...
    python utils/oracle_the_veto_yields_to_the_body_that_walks_back.py --k 50
"""
...
def neutralise(agent_module):
    """Switch the veto exemption off in `agent_module`, in place.

    The flag is read from main.py's own globals when `_promo_bet_walks_back` is
    built, so rebinding the module attribute puts BOTH match-point vetoes back
    the way they were and leaves every other promotion rule -- routes (a)-(f)
    of the selector included -- exactly as it is.
    """
...
def provenance(candidate, base):
    ...
    if (not candidate.PROMOTE_BET_OUTLIVES_MATCH_POINT
            or base.PROMOTE_BET_OUTLIVES_MATCH_POINT):
        raise SystemExit(
            "los brazos no difieren en PROMOTE_BET_OUTLIVES_MATCH_POINT: "
            f"candidato={candidate.PROMOTE_BET_OUTLIVES_MATCH_POINT} "
            f"base={base.PROMOTE_BET_OUTLIVES_MATCH_POINT}")
...
def main(argv=None):
    ...
    try:
        for b in boards:
            ...
            if not clears:
                ...
                against += 1
            print(f"{b['name']}")
            print(f"    con la salida   {b['label_with']} -> "
    ...
    print(f"\nRESUMEN: {len(boards)} tableros · {real} a favor de la salida · "
