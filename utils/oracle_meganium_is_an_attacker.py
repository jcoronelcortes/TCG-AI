"""Meganium as the wall's answer, graded by the ENGINE'S RULES instead of by a
bot we beat four times out of five.

WHY THIS EXISTS. `utils/gate_meganium_is_an_attacker_not_the_doubler.py`
measures a winrate against the reference bot, and against the Crustle lists that
bot is already losing about four games in five: a reading that changes thirteen
decisions in 3 580 cannot be separated from noise there, and it was not (the
numbers are in the write-up). So the question is asked the other way round:
...
same tree loaded twice with the two switches rebound to False in one arm, every
record replayed side by side, and every decision the two arms disagree on
becomes a board. The two boards this rule was written from (`registro_014` step
92 and `registro_020` step 137) are among them and are not privileged over the
rest.
...
Usage:
    python utils/oracle_meganium_is_an_attacker.py          # K=100
    python utils/oracle_meganium_is_an_attacker.py --k 50
"""
...
def _their_deck_for(obs, our_deck, lists):
    ...
    very Crustle this rule is about. Every list that closes is scored by how
...
def _arms(half=None):
    """The two arms. `half` NEUTRALISES the other consumer, exactly as the gate's
    own `--half` does: the two readings move different menus (the reservation
    one with a single Grass in hand, the ladder one with six) and a combined
    grade cannot say which of them paid."""
    import gate_meganium_is_an_attacker_not_the_doubler as gate

    reserve = half in (None, "reserve")
    ladder = half in (None, "ladder")
    candidate = gate.arm("oracle_arm_with", reserve, ladder)
    base = gate.arm("oracle_arm_without", False, False)
    gate.provenance(candidate, base, control=False, half=half)
...
def _corpus_boards(candidate, base):
    ...
    * `records/` is the harvested REAL games, the ones a human read to find
      this. It is where the two boards of the write-up live.
    * `tests/corpus/` is the frozen fifty. Eighteen of them ARE `crustle_wall`
      games, so unlike a rule scoped to a card nobody else plays, this one is
      expected to speak there too -- and the count is the honest measure of how
      wide the reading is rather than a leakage check.
    """
    import golden_corpus as gc
    ...
    with deck_of_record():
        ...
        _walk(gc.frozen_records(), our_deck, lists, candidate, base,
              congelados, _ung)
        print(f"  tests/corpus/ (los cincuenta congelados, 18 de ellos "
              f"crustle_wall): {len(congelados)} tableros")
        boards += congelados
...
def main(argv=None):
    ...
    ap.add_argument("--half", choices=("reserve", "ladder"), default=None,
                    help="neutralise the OTHER consumer and grade one alone")
    args = ap.parse_args(argv)

    import local_engine
    local_engine.load()

    candidate, base = _arms(args.half)
