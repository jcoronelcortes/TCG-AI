"""The reversible bet at the promotion, graded by the ENGINE'S RULES.

WHY THIS EXISTS. Route (f) of `_promote_setup_ko_attacker` (`PROMOTE_REVERSIBLE_BET`)
fires on a board that the reference bot almost never builds: our active knocked
out, a benched body ONE attachment from a lethal hit, no engine in hand to
guarantee that attachment, and an exit if the draw fails. Measured against the
Marnie bot with paired seeds it is worth nothing at all -- 2795 wins against
2796 in 3000 games -- and the firing census says why: 600 games across four
matchups produce 210 forced promotions and exactly ONE of them changes.

A winrate cannot resolve a decision that happens twice in a thousand games. So
the question is asked the other way round, the way
`utils/oracle_their_own_drip_finishes_the_body.py` asks it:
...
`PROMOTE_REVERSIBLE_BET` rebound to False in one arm, both corpora replayed side
by side, and every decision the two arms disagree on becomes a board. The record
the rule was written from (`registro_006` step 94 vs Marnie's Grimmsnarl ex) is
one of them and is not privileged over the others.
...
Usage:
    ...
    python utils/oracle_the_promotion_bets_when_it_can_walk_back.py --k 50
"""
...
def neutralise(agent_module):
    """Switch route (f) off in `agent_module`, in place.

    The flag is read from main.py's own globals inside the promotion block, so
    rebinding the module attribute switches the route off for that instance and
    leaves routes (a)-(e) exactly as they were.
    """
    agent_module.PROMOTE_REVERSIBLE_BET = False
    return agent_module
...
def provenance(candidate, base):
    ...
    if not candidate.PROMOTE_REVERSIBLE_BET or base.PROMOTE_REVERSIBLE_BET:
        raise SystemExit(
            "los brazos no difieren en PROMOTE_REVERSIBLE_BET: "
            f"candidato={candidate.PROMOTE_REVERSIBLE_BET} "
            f"base={base.PROMOTE_REVERSIBLE_BET}")
...
def main(argv=None):
    ...
    try:
        for b in boards:
            ...
            if not clears:
                ...
                against += 1
            print(f"{b['name']}")
            print(f"    con la apuesta {b['label_with']} -> "
    ...
    print(f"\nRESUMEN: {len(boards)} tableros · {real} a favor de la apuesta · "
