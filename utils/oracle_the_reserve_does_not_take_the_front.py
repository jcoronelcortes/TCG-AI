"""The reserve against the wall, graded by the ENGINE'S RULES instead of by a
bot we beat four times out of five.

WHY THIS EXISTS. `utils/gate_the_reserve_does_not_take_the_front.py` measures a
winrate against the reference bot, and against the Crustle lists that bot is
already losing about four games in five: a reading that flips two decisions in
3 580 cannot be separated from noise there. So the question is asked the other
way round:
...
same tree loaded twice with the flag rebound to False in one arm, every record
replayed side by side, and every decision the two arms disagree on becomes a
board. The board this rule was written from (`registro_006` step 58) is among
them and is not privileged over the rest.
...
Usage:
    python utils/oracle_the_reserve_does_not_take_the_front.py          # K=100
    python utils/oracle_the_reserve_does_not_take_the_front.py --k 50
"""
...
def _arms():
    ...
    import gate_the_reserve_does_not_take_the_front as gate
...
def _corpus_boards(candidate, base):
    ...
    * `records/` is the harvested REAL games, the ones a human read to find
      this. It is where the board of the write-up (registro_006 step 58) lives.
