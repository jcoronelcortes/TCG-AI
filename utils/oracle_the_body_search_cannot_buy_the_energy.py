"""The Supporter that cannot buy the energy, graded by the ENGINE'S RULES.

WHY THIS EXISTS. `utils/gate_the_body_search_cannot_buy_the_energy.py` measures
a winrate against the reference bot, and this reading changes ONE decision in
about 125 games (`utils/census_the_body_search_cannot_buy_the_energy.py`): at
that exposure a winrate cannot separate it from nothing, and the gate duly
returned +0.00 pp. So the question is asked the other way round:
...
ONLY `records/`, AND THE REASON IS THE DECK LIST. The frozen fifty flip ZERO of
their 3 580 decisions -- the corpus test says so on every run -- so there is
nothing there to grade; and they were played with the list of before 14 August
([[el-corpus-grabado-es-de-la-lista-vieja]]), while the game this rule comes
from was played today with the list on disk. Walking both here would need two
different lists in one loop for no board.

WHAT IS READ, AND WHAT IS SAMPLED. `opponent_obs=None`: a record keeps one
seat's observations. Their hand is drawn from their unseen multiset like the
prizes are, so every grade is computed against a LEGAL world rather than the
true one (`mano rival MUESTREADA` says so in the report) -- and it is the same
for both options of a board, which is what keeps the comparison fair.
...
Usage:
    python utils/oracle_the_body_search_cannot_buy_the_energy.py          # K=100
    python utils/oracle_the_body_search_cannot_buy_the_energy.py --k 50
"""
...
def _record_boards(candidate, base):
    """The boards of `records/`, played with the list on disk (see the header)."""
    import golden_corpus as gc
