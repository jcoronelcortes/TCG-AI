"""Self-play gate for the three switches of "the front spot goes to the body
that can still attack", with its own control.

    PROMOTION_READS_THE_KNOCKOUT_NOT_THE_ATTACK
...
READ THE CENSUS FIRST -- `utils/census_the_front_spot_goes_to_the_attacker.py`.
The board these rules are about is the FORCED PROMOTION, which the stored corpus
produces four times in fourteen records, and only one of those four is a board
where anything changes. At that exposure a winrate cannot resolve the change and
this gate is a HARM CHECK, not the evidence. The evidence is the census plus the
golden-corpus flip (registro_008 step 109, Tapu Bulu -> Teal Mask Ogerpon ex)
and the frozen fifty at 0 flips over 3 580 decisions.

`--control` is what says so out loud: it plays the flags against THEMSELVES, so
its delta is the noise floor at the same N and any candidate row smaller than
that row is noise
([[el-suelo-de-ruido-del-grupo-de-control-ya-es-cero]]).

`--only` measures ONE of the three, which is what to reach for when a row comes
out non-zero: the three are one sentence but they are not one mechanism, and a
gate that can only say "the sentence" cannot say which half moved.

Usage:
    python utils/gate_the_front_spot_goes_to_the_attacker.py --games 1000 \
        --opponent deck/opponents/archaludon.csv
    python utils/gate_the_front_spot_goes_to_the_attacker.py --games 1000 --control
    python utils/gate_the_front_spot_goes_to_the_attacker.py --games 1000 \
        --only PROMOTION_READS_THE_KNOCKOUT_NOT_THE_ATTACK
"""
...
def arm(name, value, only=None):
    """`main.py` with the flags under measurement bound to `value`.

    `only` narrows the measurement to a single flag; the other two keep the
    value the file ships with, so the arm still differs from its baseline by
...
def main(argv=None):
    ...
    ap.add_argument("--only", choices=FLAGS, default=None,
                    help="measure ONE of the three instead of the sentence")
    ...
    without = run(arm("arm_control", False, args.only), "baseline")
    delta, z, p = wilson_delta(with_rule, n, without, n)
    print(f"\n{'CONTROL' if args.control else 'CANDIDATO'} "
          f"({args.only or 'las tres'}, {args.opponent or 'deck.csv'}, n={n}): "
