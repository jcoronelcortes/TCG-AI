...
def _ripen_energy_capped(pokemon, ogerpon_phys_cap=None):
    ...
    if pid in (Chikorita, Applin):
        return phys >= 1
    if pid == Tapu_Bulu:
        return phys >= (2 if meganium_in_play else 4)
    if pid == Teal_Mask_Ogerpon_ex:
        if op_is_crustle_deck and phys >= 2:
            return True
...
_op_prize_denial_pecharunt = False   # Pecharunt ex (141) en el campo rival
_op_prize_denial_gengar = False      # Mega Gengar ex (772) en el campo rival
...
def prize_count_op(pokemon: Pokemon) -> int:
    ...
    if count <= 0:
        return 0
    if _op_prize_denial_pecharunt and pokemon.id == Munkidori_ex:
        count -= 1
    if _op_prize_denial_gengar:
...
def _ctx_ns_fetch(my_state, state, hand_counts, field_counts, bench_count,
    ...
    if _op_bench_snipe_dmg > 0 and hand_counts.get(Hydrapple_ex, 0) == 0:
        for _nsd in (my_state.bench or []):
            if _nsd is None or _nsd.id != Dipplin:
                continue
            if (_nsd.hp or 0) > _op_bench_snipe_dmg:
                continue  # sobrevive el goteo de este turno: no urge
            if getattr(_nsd, 'appearThisTurn', False) and not forest_in_play:
...
_REGLAS_BCS_FEZ = [
    _ReglaFija("lucario_responde",
               lambda c: (c.op_is_lucario
                          ...
                          (ko_last_turn or c.bench_count == 0)),
               lambda c: 650),
...
class _CtxPPFetch:
    ...
    def __init__(self, card_id, hand_counts, field_counts, bench_count,
        ...
        self.first_turn = ((state.turn == 1 and we_go_first) or
                           (state.turn == 2 and not we_go_first))
        ...
        if not meganium_in_play and hand_counts.get(Meganium, 0) == 0:
            if field_counts.get(Bayleef, 0) >= 1:
                has_evo = True
            elif (forest_in_play and field_counts.get(Chikorita, 0) >= 1 and
                  hand_counts.get(Bayleef, 0) >= 1):
                has_evo = True
...
_REGLAS_PP_FETCH = [
    ...
    _ReglaFija("fb_bayleef",
               lambda c: (c.card_id == Bayleef and not meganium_in_play
    ...
    _ReglaFija("fb_meganium",
               lambda c: (c.card_id == Meganium and not meganium_in_play
                          and c.hand.get(Meganium, 0) == 0 and c.have_bay),
               lambda c: 700),
    _ReglaFija("fb_chikorita",
               lambda c: (c.card_id == Chikorita and not meganium_in_play
                          and c.campo.get(Chikorita, 0)
                          + c.campo.get(Bayleef, 0)
...
def _v_meowth_fetch_valor(c):
    score = c.sv
    if c.card_id == Boss_Orders and op_is_crustle_deck:
        score += 100
    ...
    if (c.card_id == Dawn and not forest_in_play
...
_REGLAS_DAWN_MEGANIUM = [
    _ReglaFija("ya_en_juego",
               lambda c: meganium_in_play, lambda c: 10),
    _ReglaFija("evo_inmediata",
...
_REGLAS_DAWN_TAPU = [
    ...
    _ReglaFija("anti_muro_con_meganium",
               lambda c: ((op_is_crustle_deck or c.op_ex_immune_active
                           or c.op_ex_immune_bench)
                          and meganium_in_play),
               lambda c: 700),
    _ReglaFija("anti_muro",
...
_REGLAS_DAWN_FEZ = [
    ...
    _ReglaFija("tras_ko",
               lambda c: ko_last_turn, lambda c: 500),
]
...
_REGLAS_DAWN_FOREST = [
    _ReglaFija("falta_forest",
               lambda c: not forest_in_play and not _dawn_forest_avail(c),
               lambda c: 600),
]
...
def _ctx_gust_objetivo(card, o, my_state, op_state, state, hand_counts,
    ...
    if atk is not None:
        ...
        eff_dmg = _our_effective_damage(atk, card, dmg, meganium_in_play,
...
def agent(obs_dict: dict) -> list[int]:
    ...
    global _op_prize_denial_pecharunt, _op_prize_denial_gengar
    _op_field_ids = {p.id for p in ((op_state.active or [])
                                    + (op_state.bench or [])) if p is not None}
    _op_prize_denial_pecharunt = Pecharunt_ex in _op_field_ids
    _op_prize_denial_gengar = Mega_Gengar_ex in _op_field_ids
    ...
    if op_bench_snipe_threat:
        for _bs_pk in ([_active_of(op_state)] + list(op_state.bench or [])):
            ...
            if _bs_pk.id in OP_BENCH_SNIPE_DAMAGE:
                _op_bench_snipe_dmg = max(
                    _op_bench_snipe_dmg, OP_BENCH_SNIPE_DAMAGE[_bs_pk.id])
        if _op_bench_snipe_dmg == 0:
            _op_bench_snipe_dmg = OP_BENCH_SNIPE_DEFAULT
    ...
    if context == SelectContext.MAIN:
        ...
        if state.turn >= 2 and len(my_cards) > 0 and len(op_cards) > 0:
            ...
            if (_op_bench_empty and plan.attacker == 0
                    and plan.remain_hp is not None and plan.remain_hp <= 0):
                _active_win_plan = (
                    plan.attacker, plan.target, plan.attack_index,
                    plan.remain_hp, plan.energy)
            ...
            if (not _prize_denial_pivot
                ...
                if my_prize <= prize_count_op(_op_act_main):
            ...
            if _active_win_plan is not None and plan.attacker != 0:
                (plan.attacker, plan.target, plan.attack_index,
                 plan.remain_hp, plan.energy) = _active_win_plan
    ...
    if context == SelectContext.MAIN:
        ...
        if _td_act is not None and any(
                ...
                for o in select.option):
            _td_ability_serial = getattr(_td_act, 'serial', None)
    ...
    if (not state.supporterPlayed
        ...
        _mbw_td = (_mbw_atk is not None
                   ...
                   and _td_ability_serial is not None
                   and getattr(_mbw_atk, 'serial', None) == _td_ability_serial)
    ...
    if (op_is_cubchoo_deck and not can_attack
        ...
        _cls_grass_after = max(0, total_grass - _retreat_cards(_cls_rc))
    ...
    if _olf_active is not None and _olf_active.id in MAIN_ATTACKERS:
        ...
        if (_olf_active.id in (Hydrapple_ex, Teal_Mask_Ogerpon_ex)
                and _olf_a_grass):
            _olf_a_extra += _grass_attach_unit()
        if _olf_a_eff + _olf_a_extra >= ATTACK_ENERGY_REQ.get(_olf_active.id, 99):
    ...
    def energy_score(pokemon: Pokemon, active: bool) -> float:
        ...
        if active and (_win_via_boss_gust or _gust_2prize_via_boss):
            _active_extra_charge_wasted = (
                ...
                and energy_count >= ATTACK_ENERGY_REQ.get(pokemon.id, 99))
    ...
    # arma cuando la curacion CAMBIA el resultado (el cuerpo muere al golpe
    # proyectado y con +30 sobrevive), nunca como curacion cosmetica. Se calcula
    # DESPUES de `energy_score` para poder consultar sus prioridades: si alguna
    # carga vale >= 41000 hay un REMATE/pivote letal pendiente y la Planta no se
    # desvia a curar. Ante varios candidatos gana el de MENOS vida (el que muere
    # antes) y, a igualdad, el de banca (el activo tiene ademas la retirada).
    ...
    if (field_counts.get(Hydrapple_ex, 0) >= 1
        ...
        if not _rh_lethal_pending:
            ...
            for _rh_act, _rh_pk in _rh_cands:
                ...
                # el snipe automatico. Curar SALVA si ahora muere y despues no.
                _rh_thr = (estimated_op_damage if _rh_act else _op_bench_snipe_dmg)
                if _rh_thr <= 0 or _rh_thr < _rh_hp:
                    continue
                if _rh_thr >= min(_rh_max, _rh_hp + RIPENING_HEAL):
                    continue  # la curacion no basta: seguiria muriendo
                _rh_key = (_rh_hp, 1 if _rh_act else 0)
    ...
    _meowth_fetch_id, _meowth_fetch_val = _meowth_fetch_prediccion(
        ...
        CARTAS_ACTIVAS_EN_MAZO, _our_first_action_turn)
    ...
    if (_forced_ko_promote and _best_promote_card is None
        ...
        _ps_grass_total = sum(
            CARTAS_ACTIVAS_EN_MAZO.get(Basic_Grass_Energy, {}).values())
        _ps_grass_visible = (
        ...
        # Como conseguir la energia que falta el proximo turno: un Supporter de
        # robo/refresco en mano (Lillie's/Dawn) mas una Planta accesible (en la
        # mano ya, o aun oculta en mazo/premios para robarla). Sin ese motor de
        # busqueda no se privilegia al ex.
        _ps_can_find_energy = (
            (hand_counts.get(Lillie_Determination, 0) >= 1
             or hand_counts.get(Dawn, 0) >= 1)
            and (hand_counts.get(Basic_Grass_Energy, 0) >= 1
                 or _ps_grass_hidden >= 1))
        if _ps_remain > 0 and _ps_can_find_energy:
            _ps_best_key = None
            for _psb in my_state.bench:
                if _psb is None or not isinstance(_psb, Pokemon):
                    continue
                _ps_req = ATTACK_ENERGY_REQ.get(_psb.id)
                if _ps_req is None:
                    continue
                ...
                _ps_key = (-_ps_deficit, _ps_hp, _ps_dmg)
    ...
    def _promo_kos_op(_pk):
        ...
        _pbase = _attacker_base_damage(
            ...
            teal_self_energy=_pe, bench_count=bench_count)
        if _pbase <= 0:
            return False
        ...
        if o.type == OptionType.NUMBER:
            ...
            if card is not None:
                ...
                if context == SelectContext.SWITCH or context == SelectContext.TO_ACTIVE:
                    ...
                    if card.id == Basic_Grass_Energy:
                        ...
                        _forest_counters_op_stadium = (
                            (neutralization_zone_active or watchtower_in_play)
            ...
            if card is not None and pokemon is not None:
                ...
                if o.inPlayArea == AreaType.ACTIVE:
                    ...
                    _ft_veto_ids = ((Teal_Mask_Ogerpon_ex,) if op_is_crustle_deck
            ...
            if card is not None and pokemon is not None:
                ...
                if (op_is_cubchoo_deck and score > 0
                    ...
                    if _cub_evo_eff < ATTACK_ENERGY_REQ.get(card.id, 99):
                        score = SCORE_VETO
            ...
            if card is not None:
                if card.id == Teal_Mask_Ogerpon_ex:
                    ...
                    if hand_counts[Basic_Grass_Energy] < 1:
                        score = SCORE_VETO
                    elif _attach_enable_retreat_ko and o.area == AreaType.ACTIVE:
                    ...
                    if hand_counts[Basic_Grass_Energy] < 1:
                        ...
                        if _extra_energy_enables_ko(Hydrapple_ex, _hydra_energy):
                            ...
                            score = RIPEN_HEAL_ABILITY_SCORE
                        else:
                            score = SCORE_VETO
        ...
        if _sb_sterile and not _sb_wins:
            ...
            for _sbi, _sbo in enumerate(select.option):
                ...
                if (_sbc.id == Ultra_Ball and _sb_fetch_i < 0
                        and any(CARTAS_ACTIVAS_EN_MAZO.get(_b, {}).get(
                                    ESTADO_MAZO, 0) > 0
