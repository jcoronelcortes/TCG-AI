...
def test_no_architecture_violations():
    ...
    assert not failures, f"infracciones de arquitectura:\n{detail}"
...
def test_r4_catches_a_lazy_import_of_our_package(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(la, "MAIN_PY", file_path)
    monkeypatch.setattr(la, "PACKAGE", tmp_path / "ptcg")
    failures = la.rule_4_lazy_imports()
    assert [f[0] for f in failures] == ["R4"]
...
def test_r1_catches_a_mutable_imported_by_name(tmp_path, monkeypatch):
    ...
    (package / "decision").mkdir(parents=True)
    (package / "__init__.py").write_text("")
    (package / "decision" / "ub.py").write_text(
    ...
    failures = la.rule_1_imported_mutables()
    assert [f[0] for f in failures] == ["R1"]


def test_r2_catches_state_in_a_pure_module(tmp_path, monkeypatch):
    package = tmp_path / "ptcg"
    (package / "cards").mkdir(parents=True)
    (package / "__init__.py").write_text("")
    (package / "cards" / "ids.py").write_text("from ptcg.state import AGENT_STATE\n")
    monkeypatch.setattr(la, "PACKAGE", package)
    failures = la.rule_2_purity()
    assert [f[0] for f in failures] == ["R2"]


def test_r2_allows_state_in_calc(tmp_path, monkeypatch):
