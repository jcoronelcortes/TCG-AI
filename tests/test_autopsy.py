...
def test_loss_on_prizes():
    ...
    assert classify_loss(obs, asiento=0, result="pierde") == "premios"
...
def test_loss_by_bench_out():
    ...
    assert classify_loss(obs, asiento=0, result="pierde") == "bench_out"


def test_loss_by_deckout():
    obs = _obs_final(op_prize_restantes=3,
                     my_active={"id": 920}, my_bench=[], mi_deck=0)
    assert classify_loss(obs, asiento=0, result="pierde") == "deckout"
...
def test_the_boundary_is_classified_outright():
    ...
    assert classify_loss(obs, asiento=0, result="limite") == "limite"


def test_a_broken_observation_does_not_raise():
    assert classify_loss({}, asiento=0, result="pierde") == "desconocido"
    assert classify_loss(None, asiento=0, result="pierde") == "desconocido"
